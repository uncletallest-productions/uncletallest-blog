...
**SessionEnd**: Watches for token budget approaching threshold, or for keywords like "wrap up" or "save progress." When it fires, it captures session state — what was accomplished, what's in flight, what needs to carry forward — before the context window closes. The next session picks up with a handoff instead of amnesia.
